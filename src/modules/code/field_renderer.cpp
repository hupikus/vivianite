...
FieldRenderer::FieldRenderer(Field& active_field)
    : field(active_field)
{
...
void FieldRenderer::Render(int pos_x, int pos_y, size_t width, size_t height)
{
    ...
    for (const std::string& line : field.Text)
    {
        ...
        for (int line_iter = 0; line_iter < size;)
        {
            ...
            // cursor
            if (y == field.cursor_visual_y and index == field.cursor_visual_x) {
                DrawRectangle(pos_x + HORIZONTAL_MARGIN + fontSize.x * x, check_y, 2, FONT_SIZE, WHITE);
            }
...
const Color* KeywordCheck(const std::string* buffer, const CodePalette* palette, CodeState* state)
{
    ...
    // branch keywords
    if (std::find(kwbegin, kwend, *buffer) != kwend) {
        return &(palette->branch);
    }

    // logic keywords
    if (std::find(lgbegin, lgend, *buffer) != lgend) {
    ...
    if (std::find(state->vars.begin(), vend, *buffer) != vend) {
        return &(palette->variable);
    }

    // use of defined functions
    vend = state->functions.end();
    if (std::find(state->functions.begin(), vend, *buffer) != vend) {
        return &(palette->function);
    }
...
// Generate text highlight and analyse code through
        ...
        for (auto it = line.begin(); it != line.end(); ++it)
        {
            ...
            switch (*it)
            {
                ...
                case ']':
                    ...
                    {
                        current_color = &(palette->brackets[render->brackets % 5]);
                        if (--render->brackets == -1)
                        {
                            current_color = new Color{255, 0, 0, 255};
                        }
                    }
                    break;
                case ',':
