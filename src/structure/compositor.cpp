...
void Compositor::Process()
{
    ...
    // key process
    if (!IsWindowHidden() and IsWindowFocused()) {
        keys();
    } else {
        focus_tile = nullptr;
    }
    ...
    if (IsKeyPressed(KEY_N) and ctrl)
    {
        ...
        *focus_tile = std::unique_ptr<Tile>(
            new Tile{
                .vertical = shift,
                .ratio = 0.5f,
                .layout = alt ? std::array<std::unique_ptr<Tile>, 2>{std::move(created_tile), std::move(*moved_tile)} : std::array<std::unique_ptr<Tile>, 2>{std::move(*moved_tile), std::move(created_tile)}});
